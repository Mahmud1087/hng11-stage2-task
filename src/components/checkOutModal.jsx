...
const CheckOutModal = ({
  ...
  return (
    <div className='fixed top-0 w-screen h-screen bg-black/40 flex items-center justify-center'>
      <section className='relative bg-white rounded-lg w-[90%] mx-auto sm:w-4/5 lg:w-3/5 py-4 px-12 text-gray-prim'>
        ...
        <article className=' flex flex-col gap-4 lg:flex-row'>
          <aside className={`${cartData.length === 0 && 'hidden'} w-1/2`}>
            <h2>
              <span className='text-lg font-semibold pr-5'>Full Name:</span>
              {`${firstName} ${lastName}`}
            </h2>
            <h2>
              <span className='text-lg font-semibold pr-5'>Email:</span>
              {`${emailAddress}`}
            </h2>
            <h2>
              <span className='text-lg font-semibold pr-5'>
                Billing Address:
              </span>
              {`${billingAddress}`}
            </h2>
            <h2>
              <span className='text-lg font-semibold pr-5'>Phone Number:</span>
              {`${phoneNumber}`}
            </h2>
          </aside>
              ...
              <section>
                ...
                {cartData.map((item, i) => {
                  return (
                    ...
                    >
                      <h1 className='w-3/5'>{item.name}</h1>
        ...
        <Link
          ...
          } w-fit mx-auto mt-2 px-12 py-2 text-sm text-white bg-green-sec rounded-md transition-all font-semibold hover:bg-[#234149] border-2 border-green-sec`}
          onClick={() => {
            setCartData([]);
            closeModal();
          }}
